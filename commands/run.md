...
Exit codes: `0` finished, `1` startup (`CursorAgentError`), `2` run executed and failed (`error` or `cancelled`).
...
- [ ] On `error` or `cancelled`, report exit 2 (and the SDK `error` message when present) and do not pretend success
